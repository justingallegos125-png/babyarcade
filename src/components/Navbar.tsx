...
const Navbar: React.FC<NavbarProps> = ({ searchQuery, setSearchQuery, onHome, onOpenLoader, onOpenData, onOpenHelp, onOpenProxy, onOpenCustom }) => {
  return (
    <nav className="bg-black/80 border-4 border-accent px-6 py-3 flex items-center justify-between rounded-full backdrop-blur-md glow-border">
      <div className="flex items-center gap-4 cursor-pointer group" onClick={onHome}>
        ...
        <span className="text-2xl font-black tracking-tighter hidden lg:block select-none text-glow text-accent glitch text-nowrap uppercase font-graffiti">
          Joshy's Arcade
        </span>
      </div>
